...
##        print Arc1
    ...
    def calc_high_accurancy_BiarcCurve(self):
        ...
        self.min_step = self.max_step * self.epsilon
    ...
    def calc_Biarc_section(self, u_sect, nom_tol, max_tol):
        ...
        while(u < u_sect[-1] - min_u):
            ...
            if Biarc.shape == "Zero":
                ...
                if self.check_biarc_fitting_tolerance(Biarc, max_tol, cur_step, u):
                    #print("fit1")
                    PtsVec.append(PtVec)
                    BiarcCurve.append(Biarc)
                    ...
                    # print("else",cur_step)
                    u -= cur_step
                    cur_step = max(cur_step*0.7,self.min_step)

            # if step % 10 == 0:
            #     print step,cur_step,u
            if step > 10000:
                raise ValueError, "Iterations above 10000 reduce tolerance"

        return BiarcCurve, PtsVec

    def check_biarc_fitting_tolerance(self, Biarc, epsilon, cur_step, u1):
        """
        check_biarc_fitting_tolerance()
        """
        if cur_step > self.min_step:
            u0 = u1 - cur_step
            check_step = cur_step / 5
            check_u = []
            check_Pts = []
            fit_error = []
            ...
            #print "fit error : ",max(fit_error)
            if max(fit_error) >= epsilon:
                return 0
            else:
                return 1
        else:
            print "reach min seg length", cur_step
            return 1
